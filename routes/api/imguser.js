...
routerimg.post('/Post/:postId', upload.single('image'), async (req, res) => {
    try {
        console.log("Uploading ...... ")
        console.log(req)
        let { filename } = req.file;
        ...
        let date = new Date(image.uploadDate)
        console.log(image)
        let post = await Post.findOneAndUpdate({ _id:postId }, { image: image.filename }, {
            new: true
        }).exec()
        console.log(post)
        console.log('TIME NOW: ', date.getHours() - 12, ':', date.getMinutes())
        res.status(200).send({ post, image, message: "Uploaded successfully", success: true })
    } catch (error) {
        res.status(404).send({ error, message: "Unable to upload", success: false })
    }
})
